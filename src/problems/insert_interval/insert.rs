...
impl Solution {
    pub fn insert(intervals: Vec<Vec<i32>>, new_interval: Vec<i32>) -> Vec<Vec<i32>> {
        ...
        let (right, end, overlap) = match intervals.binary_search_by( |i| {
            if new_interval[1] > i[1] { Less }
            else if new_interval[1] < i[0] { Greater }
            else { Equal }
        } ) {
            Ok(right) => {
                let end = intervals[right][1];
                (right, end, true)
            },
            Err(right) => {
                if right == 0 {
                ...
                (right, new_interval[1], false)
            }
        };
        let mut left = right;
        while left > 0 && intervals[left - 1][1] >= new_interval[0] { left -= 1; }
        if left == right && !overlap {
            intervals.insert(left, new_interval);
            return intervals;
        }
        let new_interval = vec![intervals[left][0].min(new_interval[0]), end];
        for _ in left..(right.saturating_sub(1)) { intervals.remove(left); }
        if left < right && overlap { intervals.remove(left); }
        intervals[left] = new_interval;
        intervals
    }
}
