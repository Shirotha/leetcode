...
impl BitSet {
    ...
    #[inline] fn contains(&self, i: usize) -> bool {
        let (i, b) = index_mask(i);
        self.0[i] & b != 0
    }
    #[inline] fn clear(&mut self) { self.0.fill(0); }
}
...
struct Graph([GraphData; 2]);
impl Graph {
    fn new(n: i32, edges: Vec<Vec<i32>>) -> Self {
        let n = n as usize;
        let costs = vec![i32::MAX; n];
        let mut adj = Vec::new();
        adj.resize_with(n, Vec::new);
        let df = GraphData { edges: adj, queue: BinaryHeap::new(), costs: costs.clone(), closed: BitSet::new(n) };
        let mut adj = Vec::new();
        adj.resize_with(n, Vec::new);
        let db = GraphData { edges: adj, queue: BinaryHeap::new(), costs, closed: BitSet::new(n) };
        let mut g = Graph([df, db]);
    ...
    #[inline] fn add_edge(&mut self, edge: Vec<i32>) {
        ...
        self.0[0].edges[from].push(Edge(to, cost));
        self.0[1].edges[to].push(Edge(from, cost));
    }
    fn shortest_path(&mut self, node1: i32, node2: i32) -> i32 {
        if node1 == node2 { return 0; }
        let target = [node2 as usize, node1 as usize];
        for i in [0, 1] {
            let start = target[i ^ 1];
            self.0[i].queue.clear(); self.0[i].costs.fill(i32::MAX); self.0[i].closed.clear();
            self.0[i].queue.push(Item(start, 0)); self.0[i].costs[start] = 0;
        }
        ...
        loop {
            for i in [0, 1] {
                if let Some(Item(current, _)) = self.0[i].queue.pop() {
                    self.0[i].closed.insert(current);
                    node[i] = current; cost[i] = self.0[i].costs[current];
                } else { return -1; }
            }
            for i in [0, 1] {
                let ii = i ^ 1;
                for &Edge(next, step) in &self.0[i].edges[node[i]] {
                    let cost = cost[i] + step;
                    let prev = self.0[i].costs[next];
                    if !self.0[i].closed.contains(next) && cost < prev {
                        self.0[i].queue.push(Item(next, cost));
                        self.0[i].costs[next] = cost;
                    }
                    if self.0[ii].closed.contains(next) {
                        let cost = cost + self.0[ii].costs[next];
                        if cost < limit { limit = cost; }
                    } 
                }
            }
            if self.0[0].costs[node[0]] + self.0[1].costs[node[1]] >= limit { return limit; }
        }
    }
}
